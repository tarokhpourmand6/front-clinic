...
const Calendar = ({
  ...
  const handleKeyUp = ({ key }) => {
    if (key === 'Tab' && calendarElement.current) {
      calendarElement.current.classList.remove('-noFocusOutline');
    }
  };

  const el = calendarElement.current;
  if (el) el.addEventListener('keyup', handleKeyUp, false);

  return () => {
    if (el) el.removeEventListener('keyup', handleKeyUp, false);
  };
}, []);
