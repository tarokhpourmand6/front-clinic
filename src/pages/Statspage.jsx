...
export default function StatsPage() {
  ...
  const loadStats = useCallback(async () => {
    if (!authenticated) return; // گارد منطقی (نه رندر)
    setLoading(true);
    setErr("");
    try {
      const data = await getSalesStats({ group, start: startISO, end: endISO });
      setStats(data);
    } catch (e) {
  ...
  const loadInjections = useCallback(async () => {
    ...
    try {
      ...
      setInjRows(Array.isArray(data) ? data : []);
    } finally {
      setInjLoading(false);
    }
  }, [authenticated, startISO, endISO]);
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => { clearDates(); setTimeout(loadStats, 0); }}
